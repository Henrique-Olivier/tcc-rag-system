...
class FakeLLMProvider:
    ...
    def __init__(self, response: str = "resposta de teste", complete_response: str | None = None) -> None:
        self.response = response
        self.complete_response = complete_response  # reescrita e título, quando diferente da resposta
        self.calls: list[tuple[str, list[ChatMessage]]] = []

    async def complete(self, model: str, messages: list[ChatMessage]) -> str:
        self.calls.append((model, messages))
        return self.response if self.complete_response is None else self.complete_response
